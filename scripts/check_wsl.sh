...
if grep -qEi "(microsoft|wsl)" /proc/version 2>/dev/null; then
    ...
    if grep -qEi "microsoft-standard-WSL2" /proc/version 2>/dev/null; then
        ...
        echo -e "${YELLOW}[WARNING]${NC} WSL1 detected (WSL2 recommended for better Docker performance)"
        WARNINGS_FOUND=$((WARNINGS_FOUND + 1))
    fi
else
    echo -e "${YELLOW}[WARNING]${NC} Not running on WSL (this script is designed for WSL environments)"
    exit 0
fi
...
if systemctl --version &> /dev/null; then
    ...
    if [ -f /etc/wsl.conf ]; then
        if grep -q "systemd=true" /etc/wsl.conf 2>/dev/null; then
            ...
            echo -e "${YELLOW}[WARNING]${NC} systemd not explicitly enabled in /etc/wsl.conf"
            WARNINGS_FOUND=$((WARNINGS_FOUND + 1))
        fi
    else
        echo -e "${YELLOW}[WARNING]${NC} /etc/wsl.conf not found"
        WARNINGS_FOUND=$((WARNINGS_FOUND + 1))
    fi
else
...
if command -v docker &> /dev/null; then
    ...
    if docker ps &> /dev/null; then
        ...
        echo -e "${RED}[X]${NC} Docker daemon is not running"
    ...
    if groups | grep -q docker; then
        ...
        echo -e "${YELLOW}[WARNING]${NC} User is not in docker group"
...
if (( TOTAL_RAM >= 16 )); then
    ...
    echo -e "${YELLOW}[WARNING]${NC} RAM: ${TOTAL_RAM}GB (minimum for testing, 16GB+ recommended)"
    WARNINGS_FOUND=$((WARNINGS_FOUND + 1))
else
    echo -e "${RED}[X]${NC} RAM: ${TOTAL_RAM}GB (insufficient - need 11GB minimum)"
...
if (( AVAILABLE_SPACE >= 35 )); then
    ...
    echo -e "${RED}[X]${NC} Disk space: ${AVAILABLE_SPACE}GB (need 35GB minimum)"
...
if (( CPU_CORES >= 4 )); then
    ...
    echo -e "${YELLOW}[WARNING]${NC} CPU cores: $CPU_CORES (4+ recommended)"
    WARNINGS_FOUND=$((WARNINGS_FOUND + 1))
fi
...
if [ -d "/mnt/c" ]; then
    ...
    echo -e "${YELLOW}[WARNING]${NC} Windows filesystem not mounted"
    WARNINGS_FOUND=$((WARNINGS_FOUND + 1))
fi
...
if [ -f /proc/sys/fs/binfmt_misc/WSLInterop ]; then
    ...
    echo -e "${YELLOW}[WARNING]${NC} WSL interop is disabled"
    WARNINGS_FOUND=$((WARNINGS_FOUND + 1))
fi
...
if [ $ISSUES_FOUND -eq 0 ] && [ $WARNINGS_FOUND -eq 0 ]; then
    ...
    echo -e "${RED}[X] $ISSUES_FOUND critical issue(s) found${NC}"
